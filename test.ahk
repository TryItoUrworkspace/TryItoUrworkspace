...
;If you changed any keybinds be sure to change [F] to the new Key for UI
...
;If you changed any keybinds be sure to change [E] to the new Key for UI
    ...
    Gui Add, GroupBox, cWhite x200 y335 w340 h30, Credits
        Gui Add, Text, cGreen x205 y350 w340 h50, HappyBot v3.4 - Developed by XEAHT#3193 and FlorasSecret#9666
...
Calculate:
    ...
    box := (ErrorLevel = 0) ? 1 : 2
    PixelSearch, Ax, Ay, x8, y8, x9, y9, 0x058341, 0, Fast|RGB
 
    if (ErrorLevel = 0) {
        if (box = 2) {
            x2 := Ax - (200 * B55), y2 := Ay + (20 * Y55)
            x3 := Ax + (160 * B55), y3 := Ay + (170 * Y55)
            x4 := Ax + (5 * B55), y4 := Ay + (195 * Y55)
            x5 := Ax + (160 * B55), y5 := Ay + (430 * Y55)
            x6 := Ax - (200 * B55), y6 := Ay + (195 * Y55)
            x7 := Ax - (30 * B55), y7 := Ay + (430 * Y55)
            x16 := Ax - (200 * B55), y16 := Ay + (20 * Y55)
            x17 := Ax + (160 * B55), y17 := Ay + (430 * Y55)
        }
        else {
            x2 := Ax - (100 * B55), y2 := Ay + (10 * B55)
            x3 := Ax + (80 * Y55), y3 := Ay + (85 * Y55)
            x4 := Ax + (2.5 * B55), y4 := Ay + (97.5 * Y55)
            x5 := Ax + (80 * B55), y5 := Ay + (227.7 * Y55)
            x6 := Ax - (100 * B55), y6 := Ay + (97.5 * Y55)
            x7 := Ax - (15 * B55), y7 := Ay + (227.7 * Y55)
            x16 := Ax - (117.6 * B55), y16 := Ay + (10 * Y55)
            x17 := Ax + (94.11 * B55), y17 := Ay + (227.7 * Y55)
        }
        ...
        if (ErrorLevel = 1) {
            ...
            if (ErrorLevel = 0) {
                if (box = 2) {
                    x2 := Ax - (175 * B55), y2 := Ay + (65 * Y55)
                    x3 := Ax + (185 * B55), y3 := Ay + (185 * Y55)
                    x4 := Ax + (30 * B55), y4 := Ay + (215 * Y55)
                    x5 := Ax + (185 * B55), y5 := Ay + (430 * Y55)
                    x6 := Ax - (175 * B55), y6 := Ay + (215 * Y55)
                    x7 := Ax - (5 * B55), y7 := Ay + (430 * Y55)
                    x16 := Ax - (175 * B55), y16 := Ay + (65 * Y55)
                    x17 := Ax + (185 * B55), y17 := Ay + (430 * Y55)
                }
                else {
                    x2 := Ax - (87.5 * B55), y2 := Ay + (35 * B55)
                    x3 := Ax + (92.5 * Y55), y3 := Ay + (92.5 * Y55)
                    x4 := Ax + (15 * B55), y4 := Ay + (107.5 * Y55)
                    x5 := Ax + (92.5 * B55), y5 := Ay + (215 * Y55)
                    x6 := Ax - (87.5 * B55), y6 := Ay + (107.5 * Y55)
                    x7 := Ax - (2.5 * B55), y7 := Ay + (215 * Y55)
                    x16 := Ax - (87.5 * B55), y16 := Ay + (35 * Y55)
                    x17 := Ax + (92.5 * B55), y17 := Ay + (215 * Y55)
                }
            }
        }
    }
...
SearchBot:
    ; PixelSearch for the first color
    PixelSearch, vx, vy, x16, y16, x17, y17, 0xF66208, 0, Fast|RGB
    
    ; Check if PixelSearch was successful
    if (ErrorLevel = 0) {
        ; PixelSearch for the second color
        PixelSearch, px, py, vx, vy, x17, y17, 0xFF221C, 3, Fast|RGB
        
        ; Check if the second PixelSearch was successful
        if (ErrorLevel = 0) {
            ubp := 1
            SetTimer, Release, -1000
            GoSub ubparry
        } else {
            ; Check if Blackprior is set
            if (Blackprior = 1) {
                GoSub Dodge1
            } else {
                GoSub Dodge2
...
AutoBlock:
    ; PixelSearch for the specified color
    ...
    if (Errorlevel = 0) {
        ...
        if (zx > x4 and zy > y4) {
            GoSub RGT
        } else if (zx < x7 and zy > y4) {
            GoSub LFT
        } else if (zy > y2 and zy < y3) {
            GoSub TOP
        }
    }
...
AutoGB:
       if(Active21=1){
      PixelSearch,tx,ty,550,550,650,650,0xBB2C26,5,Fast|RGB
      if(Errorlevel = 0) {
...
ubparry:
    Loop {
        ...
        ; Check if the first PixelSearch was successful
        if (ErrorLevel = 1) {
            Sleep, Pause1
            ...
            ; Check if the second PixelSearch was successful and Active1 is set
            if (ErrorLevel = 0 && Active1 = 1) {
                ...
                ; Perform appropriate action based on pbp value
                if (pbp = 0) {
                    Click, Right
                } else if (pbp = 1) {
                    Click, Left
...
Dodge1:
    if (Active1 = 1) {
        ...
        ; If "W" key is not held down, perform a jump
        Click, Left
...
TOP:
    ...
    While GetKeyState("F") {
        PixelSearch, px, py, x16, y16, x17, y17, 0xFD1D1A, 0, Fast|RGB
        if (ErrorLevel = 1) {
            PixelSearch, px, py, x16, y16, x17, y17, 0xFFB3AB, 0, Fast|RGB
            if (ErrorLevel = 0) {
    ...
    While GetKeyState("E") {
        ...
        if (ErrorLevel = 1) {
            ...
            ; Check if the second PixelSearch was successful
            if (ErrorLevel = 0) {
                ; Check Active conditions and take actions accordingly
                if (Active9 = 1) {
                    Send, {RButton}
                    Sleep, 850
                    return
        ...
        if (ErrorLevel = 1) {
            ...
            if (ErrorLevel = 0) {
                ...
                if (Active12 = 1) {
                    ...
                    BlockInput, On
                    Send, {W Up}{S Up}{A Up}{D Up}
                    Sleep, Left ; Replace with appropriate sleep time
        ...
        if (ErrorLevel = 1) {
            ...
            if (ErrorLevel = 0) {
                ...
                if (Active12 = 1) {
                    ...
                    BlockInput, On
                    Send, {W Up}{S Up}{A Up}{D Up}
                    Sleep, 100 ; Replace with appropriate sleep time
                    Click, Right
                    Send, {Space Down}{Space Up}
                    BlockInput, Off
                    SetKeyDelay, 30
